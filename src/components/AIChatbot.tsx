...
const AIChatbot: React.FC<AIChatbotProps> = ({ onRecommendation }) => {
    ...
    if (!isOpen) {
        return (
            <motion.button
                ...
                className="fixed bottom-6 right-6 z-40 bg-indigo-600 dark:bg-indigo-500 text-white p-4 rounded-full shadow-lg shadow-indigo-300 dark:shadow-indigo-900/50 flex items-center gap-3 pr-6 hover:bg-indigo-700 dark:hover:bg-indigo-400 transition-colors"
            >
                <div className="relative">
                    <Bot size={24} />
                    <span className="absolute top-0 right-0 w-3 h-3 bg-green-400 border-2 border-indigo-600 dark:border-indigo-500 rounded-full"></span>
                </div>
    ...
    return (
        <motion.div
            ...
            className="fixed bottom-6 right-6 z-40 w-[90vw] max-w-[380px] bg-white dark:bg-slate-900 rounded-3xl shadow-2xl shadow-slate-300/50 dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col max-h-[600px]"
        >
            {/* Header */}
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 dark:from-indigo-700 dark:to-blue-800 p-4 flex justify-between items-center text-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950/50 min-h-[300px]">
                {messages.map((msg) => (
                    ...
                    >
                        {msg.sender === 'bot' && (
                            <div className="w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center mr-2 mt-1 shrink-0 text-indigo-600 dark:text-indigo-400">
                                <Bot size={14} />
                            </div>
                        )}
                        <div
                ...
                {isTyping && (
                    <div className="flex justify-start">
                        <div className="w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center mr-2 text-indigo-600 dark:text-indigo-400">
                            <Bot size={14} />
                        </div>
                    ...
                    {messages[messages.length - 1].sender === 'bot' &&
                        ...
                        messages[messages.length - 1].options?.map((opt) => (
                            <button
                                ...
                                className="px-4 py-2 bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 text-xs font-semibold rounded-xl hover:bg-indigo-100 dark:hover:bg-slate-700 transition-colors border border-indigo-100 dark:border-slate-700 flex items-center gap-1 active:scale-95"
                            >
                                {opt.label}
                                <ArrowRight size={12} />
                            </button>
